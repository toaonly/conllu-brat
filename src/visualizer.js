...
import { URLHash } from './url_monitor'
...
Util.profileStart('init');
        ...
        var canvasWidth = that.forceWidth || $svgDiv.width();
...
//                 };
            ...
            if (rowIndex == leftRow) {
              ...
              if (smoothArcCurves) {
                ...
                var line = path.line(cornerx, -height).
        ...
        $.each(rows, function(rowId, row) {
          ...
          if (row.sentence) {
            ...
            if (sentComment) {
              ...
              var shadowRect = svg.rect(sentNumGroup,
...
Util.profileStart('finish');
        ...
        width = highlightGroup.getBBox().width + 54;
        
        $svg.width(width);
        backgroundGroup.childNodes.forEach(rect => {
...
Util.profileStart('before render');
      ...
      if (!Visualizer.areFontsLoaded) {
        ...
        // WebFont && WebFont.load(webFontConfig);
        // setTimeout(function() {
        //   if (!Visualizer.areFontsLoaded) {
